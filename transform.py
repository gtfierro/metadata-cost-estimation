...
def gensym():
    ...
    return 'shape' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=4))
...
def defn_to_shape(shapename, varname, defn, shape_graph, ns, path = None):
    ...
    if isinstance(defn, str):
        ...
        if "union" in defn:
            return union_to_shape(shapename, shape_graph, varname, defn.pop("union"), ns)
        # handle choice if it is present
        if "choice" in defn:
            return choice_to_shape(shapename, shape_graph, varname, defn.pop("choice"), ns)
...
def string_to_shape(shapename: URIRef, shape_graph: Graph, varname: URIRef, string_value: str, path = None):
    """
...
def union_to_shape(shapename: URIRef, shape_graph: Graph, varname: URIRef, union: Dict[str, Any], ns):
    """
    "union": [
    "AFDD_rule_a",
    "AFDD_rule_b
...
def choice_to_shape(shapename: URIRef, shape_graph: Graph, varname: URIRef, choice: Dict[str, Any], ns):
    """
    "choice": [
    {"hasPoint": "Chilled_Water_Valve_Command"},
    {"hasPart": {"Chilled_Water_Valve": {"hasPoint": "Valve_Command"}}}
...
def prop_to_shape(shapename: URIRef, shape_graph: Graph, varname: URIRef, propname: URIRef, value: Any, ns):
    """
    ...
    def _consume_edges(vdict, edge_stack):
        ...
        if isinstance(value, dict):
            ...
            return defn_to_shape(shapename, varname, value, shape_graph, ns, path=property_path)
        # if value is a string, treat it like a property
        property_path = edge_list_to_property_path(edge_stack, shape_graph)
        return string_to_shape(shapename, shape_graph, varname, value, path=property_path)
